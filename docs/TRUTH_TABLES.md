...
| **Config: A8_KEY** | ⚠️ DRIFT | Code supports it, **ENV VAR NOT SET** |
...
| **Protocol: Authorization** | ⚠️ PARTIAL | Code supports Bearer token, but A8_KEY not configured |
...
|-------|----------|-------------|
| A8_KEY not configured | HIGH | Add `A8_KEY` secret to environment |
...
|-----|----------------|---------------------|----------------------|
| A2 scholarship_api | ⚠️ Partial (A8_KEY missing) | ✅ /health, /ready | ✅ persisted:true |

## Next Actions

1. **CRITICAL**: Configure `A8_KEY` secret in A2 environment
