...
class SecretsPostureManager:
    ...
    async def validate_codebase_secrets(self, scan_paths: Optional[List[str]] = None) -> List[SecretValidation]:
                ...
                for pattern in patterns:
                    ...
                    for match in matches:
                        ...
                        violation = SecretValidation(
                            ...
                            remediation=f"Move secret to environment variable or secret management system"
                        )
    ...
    async def validate_log_secrets(self, log_directories: Optional[List[str]] = None) -> List[SecretValidation]:
        """Scan logs for exposed secrets"""
        if log_directories is None:
            log_directories = ["/tmp/logs", "logs/", "."]
        
        logger.info(f"📜 Scanning logs for secret exposure")
