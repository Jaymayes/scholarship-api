...
from services.lead_routing_engine import Lead, LeadStage
...
class StageGateCriteria:
    ...
    min_score: int | None = None # Minimum qualification score
...
class DealActivity:
    ...
    sentiment_score: int | None = None  # 1-10 scale
...
class PipelineManagementSystem:
    ...
    def _can_advance_to_stage(self, deal: Deal, target_stage: DealStage) -> bool:
        ...
        return not (criteria.min_score and deal.qualification_score < criteria.min_score)
    ...
    def _get_next_stage(self, current_stage: DealStage) -> DealStage | None:
    ...
    def _calculate_deal_health(self, deal: Deal) -> DealHealth:
        ...
        if days_since_activity > deal.stale_threshold_days:
            return DealHealth.STALLED
        if deal.stage_duration_days > deal.expected_stage_duration * 1.5 or deal.overdue_tasks > 2:
    ...
    def get_pipeline_metrics(self, owner_id: str | None = None) -> PipelineMetrics:
