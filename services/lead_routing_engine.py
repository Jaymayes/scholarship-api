...
class SalesRep:
    ...
    max_acv: Optional[Decimal] = None
    min_acv: Optional[Decimal] = None
...
class LeadRoutingEngine:
    ...
    def _find_best_assignment(self, lead: Lead) -> Tuple[Optional[SalesRep], str]:
    ...
    def _lead_matches_rule(self, lead: Lead, rule: RoutingRule) -> bool:
        ...
        # Check source
        if rule.sources and lead.source not in rule.sources:
            return False
        
        return True
    
    def _find_available_rep(self, lead: Lead, rule: RoutingRule) -> Optional[SalesRep]:
    ...
    def _rep_can_handle_lead(self, rep: SalesRep, lead: Lead) -> bool:
        ...
        if rep.min_acv and lead.estimated_acv < rep.min_acv:
            return False
        if rep.max_acv and lead.estimated_acv > rep.max_acv:
            return False
